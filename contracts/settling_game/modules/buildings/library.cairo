...
namespace Buildings {
    ...
    func get_realm_buildable_area{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        cities: felt, regions: felt
    ) -> (buildable_area: felt) {
        // Get buildable units
    ...
    func get_decayed_building_time{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        time_balance: felt, decay_rate: felt
    ) -> (effective_building_time: felt) {
    ...
    }(building_id: felt, time_balance: felt, block_timestamp: felt) -> (effective_buildings: felt) {
        alloc_locals;
    ...
    func add_time_to_buildings{
        ...
        current_buildings: RealmBuildings, building_id: felt, time_stamp: felt, time_to_add: felt
        ...
        if (building_id == RealmBuildingsIds.Barracks) {
            let (time) = add_building_integrity(
                time_stamp, current_buildings.Barracks, time_to_add
            );
            buildings[5] = time;
            tempvar range_check_ptr = range_check_ptr;
            tempvar syscall_ptr: felt* = syscall_ptr;
