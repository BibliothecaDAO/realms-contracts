...
func fetch_crypt_data{
    ...
    # Data is chunked in 8-bit portions. So each variable will take 8 bits even if it's 0/1
    let (resource) = unpack_data(data, 0, 255)      # uint256 - resource generated by this dungeon (23-28)
    let (environment) = unpack_data(data, 8, 255)   # uint256 - environment of the dungeon (0-6)
    let (legendary) = unpack_data(data, 16, 255)     # uint256 - flag if dungeon is legendary (0/1)
    let (size) = unpack_data(data, 24, 255)          # uint256 - size (e.g. 6x6) of dungeon. (6-25)
    let (num_doors) = unpack_data(data, 32, 255)    # uint256 - number of doors (0-12)
    let (num_points) = unpack_data(data, 40, 255)   # uint256 - number of points (0-12)
    let (affinity) = unpack_data(data, 48, 255)     # uint256 - affinity of this dungeon (0-55)
